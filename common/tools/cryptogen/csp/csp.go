...
// LoadPrivateKey loads a private key from file in keystorePath
func LoadRSAPrivateKey(keystorePath string) (bccsp.Key, crypto.Signer, error) {
	var err error
	var priv bccsp.Key
	var s crypto.Signer
	...
	walkFunc := func(path string, info os.FileInfo, err error) error {
		if strings.HasSuffix(path, "_sk") {
			...
			priv, err = csp.KeyImport(block.Bytes, &bccsp.RSAPrivateKeyImportOpts{Temporary: true})
			if err != nil {
				return err
			}
...
func GetRSAPublicKey(priv bccsp.Key) (*rsa.PublicKey, error) {
	...
	// unmarshal using pkix
	rsaPublicKey, err := x509.ParsePKIXPublicKey(pubKeyBytes)
	if err != nil {
		return nil, err
	}
	return rsaPublicKey.(*rsa.PublicKey), nil
}
