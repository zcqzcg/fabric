...
func testGenerateLocalRSAMSP(t *testing.T, nodeOUs bool) {
	cleanup(testDir)

	err := msp.GenerateLocalRSAMSP(testDir, testName, nil, &ca.CA{}, &ca.CA{}, msp.PEER, nodeOUs)
	assert.Error(t, err, "Empty CA should have failed")
	...
	// generate signing CA
	signCA, err := ca.NewRSACA(caDir, testCAOrg, testCAName, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode)
	assert.NoError(t, err, "Error generating CA")
	// generate TLS CA
	tlsCA, err := ca.NewRSACA(tlsCADir, testCAOrg, testCAName, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode)
	assert.NoError(t, err, "Error generating CA")
	...
	// generate local MSP for nodeType=PEER
	err = msp.GenerateLocalRSAMSP(testDir, testName, nil, signCA, tlsCA, msp.PEER, nodeOUs)
	assert.NoError(t, err, "Failed to generate local MSP")
	...
	// generate local MSP for nodeType=CLIENT
	err = msp.GenerateLocalRSAMSP(testDir, testName, nil, signCA, tlsCA, msp.CLIENT, nodeOUs)
	assert.NoError(t, err, "Failed to generate local MSP")
	// check all
	for _, file := range mspFiles {
		assert.Equal(t, true, checkForFile(file),
	...
	assert.Error(t, err, "Should have failed with CA name 'test/fail'")
	signCA.Name = "test/fail"
	err = msp.GenerateLocalRSAMSP(testDir, testName, nil, signCA, tlsCA, msp.ORDERER, nodeOUs)
	assert.Error(t, err, "Should have failed with CA name 'test/fail'")
	t.Log(err)
	cleanup(testDir)
}

func TestGenerateLocalRSAMSPWithNodeOU(t *testing.T) {
	testGenerateLocalRSAMSP(t, true)
}

func TestGenerateLocalRSAMSPWithoutNodeOU(t *testing.T) {
	testGenerateLocalRSAMSP(t, false)
}
...
func testGenerateVerifyingRSAMSP(t *testing.T, nodeOUs bool) {
	caDir := filepath.Join(testDir, "ca")
	tlsCADir := filepath.Join(testDir, "tlsca")
	mspDir := filepath.Join(testDir, "msp")
	// generate signing CA
	...
	err = msp.GenerateVerifyingRSAMSP(mspDir, signCA, tlsCA, nodeOUs)
	assert.NoError(t, err, "Failed to generate verifying MSP")
...
func TestGenerateVerifyingRSAMSPWithNodeOU(t *testing.T) {
	testGenerateVerifyingRSAMSP(t, true)
}

func TestGenerateVerifyingRSAMSPWithoutNodeOU(t *testing.T) {
	testGenerateVerifyingRSAMSP(t, true)
}
