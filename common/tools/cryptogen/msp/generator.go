...
func GenerateLocalRSAMSP(baseDir, name string, sans []string, signCA *ca.CA,
	...
	*/
	...
	cert, err := signCA.SignCertificateRSA(filepath.Join(mspDir, "signcerts"),
		name, ous, nil, ecPubKey, x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{})
	if err != nil {
		return err
	}
	...
	*/
	...
	// generate X509 certificate using TLS CA
	_, err = tlsCA.SignCertificateRSA(filepath.Join(tlsDir),
		name, nil, sans, tlsPubKey, x509.KeyUsageDigitalSignature|x509.KeyUsageKeyEncipherment,
		[]x509.ExtKeyUsage{x509.ExtKeyUsageServerAuth, x509.ExtKeyUsageClientAuth})
	if err != nil {
		return err
...
func GenerateVerifyingRSAMSP(baseDir string, signCA *ca.CA, tlsCA *ca.CA, nodeOUs bool) error {
	...
	bcsp := factory.GetDefault()
	priv, err := bcsp.KeyGen(&bccsp.RSA2048KeyGenOpts{Temporary: true})
	ecPubKey, err := csp.GetRSAPublicKey(priv)
	if err != nil {
		return err
	}
	_, err = signCA.SignCertificateRSA(filepath.Join(baseDir, "admincerts"), signCA.Name,
		nil, nil, ecPubKey, x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{})
	if err != nil {
		return err
	}
