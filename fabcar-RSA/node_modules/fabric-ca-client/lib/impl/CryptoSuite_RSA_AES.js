...
//TODO
	...
	sign(key, digest) {
        ...
        const signKey = new KEYUTIL.getKey(key._key.prvKeyHex);
		// let sig = this._ecdsa.sign(digest, signKey);
        let sig = signKey.sign(Buffer.from(digest));
		logger.debug('rsa signature: ', sig);
		const der = utils.pemToDER(utils.byteToNormalizedPEM(sig));
		logger.debug('========= rsa sig der: =========',der);
		// const der = sig.toDER();
