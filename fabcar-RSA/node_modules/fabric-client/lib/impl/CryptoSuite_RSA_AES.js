...
const logger = utils.getLogger('crypto_rsa_aes');

/**
 * The {@link module:api.CryptoSuite} implementation for RSA, and AES algorithms using software key generation.
 ...
 */
class CryptoSuite_RSA_AES extends api.CryptoSuite {
//TODO
	/**
	 * constructor
	 *
	 * @param {number} keySize Key size for the RSA algorithm, can only be 256 or 384
	 * @param {string} hash Optional. Hash algorithm, supported values are "SHA2" and "SHA3"
	 */
	constructor(keySize, hash) {
	    logger.info("!!!!!!CryptoSuite_RSA_AES ACTIVE!!!!!!!");
		if (!keySize) {
        ...
        // TODO from here to below is ecc, how can we get it to rsa?
	...
	generateEphemeralKey() {
		const pair = KEYUTIL.generateKeypair('RSA', this._curveName);
		return new RSAKey(pair.prvKeyObj);
	}
	...
	async generateKey(opts) {
		...
		if (typeof opts !== 'undefined' && typeof opts.ephemeral !== 'undefined' && opts.ephemeral === true) {
			...
			const key = new RSAKey(pair.prvKeyObj);
	...
	importKey(pem, opts) {
        ...
        // console.log(key)
		if (key && key.type && key.type === 'RSA') {
                theKey = new RSAKey(key);
                logger.debug('importKey - have the key %j', theKey);
		} else {
			error = new Error('Does not understand PEM contents other than RSA private keys and certificates');
		}
	...
	sign(key, digest) {
        ...
        const jsrsaSign = ()=> {
            const signKey = key._key;
            const signature = signKey.signWithMessageHash(digest.toString('hex'),'sha256');
            return jsrsa.hextoArrayBuffer(signature)
        };
        const signature  =  jsrsaSign();


        // console.log(signature.toString('hex'), typeof signature);
        return Buffer.from(signature); // string
	...
	verify(key, signature, digest) {
        ...
        const pubKey = new KEYUTIL.getKey(key.getPublicKey()._key.pubKeyHex);
		// note that the signature is generated on the hash of the message, not the message itself
		return pubKey.verify(Buffer.from(this.hash(digest)), signature);
	}
...
function makeRealPem(pem) {
	...
	if (typeof pem === 'string') {
		...
		result = result.replace(/-----([^-]+) RSA ([^-]+)-----([^-]*)-----([^-]+) RSA ([^-]+)-----/, '-----$1 RSA $2-----$3-----$4 RSA $5-----');
	}
	return result;
}

module.exports = CryptoSuite_RSA_AES;
